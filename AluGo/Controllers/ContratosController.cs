...
namespace AluGo.Controllers
{
    ...
    public class ContratosController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<object>>> Get([FromQuery] bool? ativo, [FromQuery] Guid? imovelId, [FromQuery] Guid? locatarioId)
        {
            ...
            .AsQueryable();
            if (ativo.HasValue) q = q.Where(c => c.Ativo == ativo);
            if (imovelId.HasValue) q = q.Where(c => c.ImovelId == imovelId);
            if (locatarioId.HasValue) q = q.Where(c => c.LocatarioId == locatarioId);
        ...
        public async Task<ActionResult<Contrato>> Create(ContratoCreateDto dto)
        {
            ...
            if (imovel is null || locatario is null) return BadRequest("Imóvel/Locatário inválido.");
